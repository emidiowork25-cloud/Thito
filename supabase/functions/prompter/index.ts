...
<style>
  /* A fonte do exibidor viaja embutida: o preview do editor e a tela do celular
     têm que quebrar linha no mesmo lugar, e no meio de um programa ao vivo não
     dá para depender de rede. */
  @font-face {
    font-family: 'Figtree';
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  html, body {
    ...
    font-family: 'Creato Display', 'Figtree', "Segoe UI", system-ui, sans-serif;
  ...
  #trilho p.cue {
    color: #d9a04a; font-weight: 400; font-style: italic;
    opacity: .85; font-size: .62em;
  }
  body.claro #trilho p.cue { color: #8a5f16; }
  ...
  .anel { width: 62px; height: 62px; border-radius: 50%; border: 2px solid #123; border-top-color: #5eb3c4; animation: girar 1.1s linear infinite; }
  @keyframes girar { to { transform: rotate(360deg); } }
  #espera h1 { font-size: 15px; font-weight: 600; letter-spacing: .22em; margin: 0; color: #9db4bd; }
  #espera p { font-size: 13px; color: #6c8791; margin: 0; max-width: 30ch; line-height: 1.6; }
  ...
  #barra {
    ...
    color: #6c8791; background: linear-gradient(#000c, transparent);
    transition: opacity .4s; font-family: ui-monospace, monospace;
  }
  #barra.sumiu { opacity: 0; }
  body.claro #barra { color: #6c8791; background: linear-gradient(#fffc, transparent); }
  .luz { display: inline-block; width: 7px; height: 7px; border-radius: 50%; background: #e0656b; margin-right: 6px; }
  .luz.on { background: #4bb391; }
