@echo off
chcp 65001 >nul
title JARBAS — abrir junto com o Windows
cd /d "%~dp0"

rem ---------------------------------------------------------------------------
rem  Coloca um atalho do start.bat na pasta de Inicializar do Windows, para o
rem  JARBAS ser a primeira coisa que abre quando voce liga o computador.
rem  Para desfazer, rode este arquivo de novo e escolha remover.
rem ---------------------------------------------------------------------------

set "STARTUP=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup"
set "ATALHO=%STARTUP%\JARBAS.lnk"
set "ALVO=%~dp0start.bat"

echo.
if exist "%ATALHO%" (
  echo   O JARBAS JA abre junto com o Windows.
  echo.
  choice /c SN /m "   Quer REMOVER a abertura automatica? [S/N]"
  if errorlevel 2 goto :fim
  del "%ATALHO%"
  echo.
  echo   Pronto. O JARBAS nao abre mais sozinho.
  goto :fim
)

echo   Isso vai fazer o JARBAS abrir sozinho toda vez que voce ligar o PC.
echo.
...
powershell -NoProfile -Command ^
  ...
  "$s.WindowStyle = 7;" ^
  "$s.Description = 'JARBAS — Hub Pessoal';" ^
  "$s.Save()"

if exist "%ATALHO%" (
  echo.
  echo   Pronto. Da proxima vez que ligar o computador, o JARBAS abre sozinho.
) else (
  echo.
